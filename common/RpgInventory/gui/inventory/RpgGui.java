...
public class RpgGui extends GuiContainer {
    ...
    public void actionPerformed(GuiButton button) {
        ...
        if (button.id == 0) {
            ...
            EntityPlayer p = Minecraft.getMinecraft().thePlayer;
            if (mod_RpgInventory.proxy.getInventory(p.username).getCrystal() != null) {
                if (mod_RpgInventory.proxy.getInventory(p.username).getCrystal().getItemDamage() > 0) {
                    System.out.println("sent");
                    mod_RpgInventory.proxy.openGUI(p, 3);
                }
            }
