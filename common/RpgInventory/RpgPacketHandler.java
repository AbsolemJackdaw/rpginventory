...
public class RpgPacketHandler implements IPacketHandler {
    ...
    private void handleRawInventory(Packet250CustomPayload packet, Player player) {
        ...
        try {
            ...
            if (nbt.hasKey("pet") && nbt.getBoolean("pet") == true) {
                RpgInv inv = new RpgInv(nbt.getString("username"));
                if(inv != null && inv.getCrystal() != null){
                    nbt.setBoolean("pet", false);
                    ItemStack tmp = inv.getCrystal();
                    tmp.setTagCompound(nbt);
                    tmp.setItemName(tmp.getTagCompound().getString("PetName"));
                    inv.setInventorySlotContents(6, tmp);
                }
            } else {
                RpgInv inv = new RpgInv(nbt.getString("username"));
