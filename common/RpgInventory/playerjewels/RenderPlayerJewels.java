...
import RpgPlusPlus.weapons.skull.NecroSkull;
...
public class RenderPlayerJewels extends RenderPlayer {
	...
	protected void renderSpecialsRpg(EntityPlayer player, float par2) {
		...
		if (shield != null) 
		{
			...
			{
				if(shield.getItem() == mod_RpgInventory.daggers) {
				...
				if (shield.getItem() == mod_RpgInventory.beastShield)
				{
			...
			if (shield.getItem() == mod_RpgInventory.archersShield) {
			...
			if (shield.getItem() == mod_RpgInventory.talisman) {
			...
			// bypass import for mod_RpgPlus
			 if (mod_RpgInventory.hasRpg && shield.getItem() == mod_RpgInventory.necro_shield) 
			{
			...
			}  if (mod_RpgInventory.hasRpg && shield.getItem() == mod_RpgInventory.pala_shield) {
			 ...
			 if (shield.getItem() == mod_RpgInventory.berserkerShield)
			{
			...
			if(mod_RpgInventory.hasShields == true)
			{
