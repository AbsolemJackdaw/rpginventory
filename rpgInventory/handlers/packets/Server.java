...
public class Server {
	...
	public static void sendData(EntityPlayerMP player, int index, Object... obs) {
		try {
			mod_RpgInventory.Channel.sendTo(
					new FMLProxyPacket(getBytes(index, obs), "RpgInv"), player);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public static void sendToAll(int index, Object... obs) {
		try {
			mod_RpgInventory.Channel.sendToAll(new FMLProxyPacket(getBytes(
					index, obs), "RpgInv"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static void sendToAllAround(Entity entity, int index, Object... obs) {
		try {
			TargetPoint point = new TargetPoint(entity.dimension, entity.posX,
					entity.posY, entity.posZ, 60);
			mod_RpgInventory.Channel.sendToAllAround(new FMLProxyPacket(
					getBytes(index, obs), "RpgInv"), point);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
