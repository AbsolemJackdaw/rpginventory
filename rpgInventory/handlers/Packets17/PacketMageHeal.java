		...
		if (!player.worldObj.isRemote) {
			...
			if (CommonTickHandler.globalCooldownMap.get(player.getCommandSenderName()) <= 0) {
				...
				if ((item.getItemDamage() + 3) >= item.getMaxDamage()) {
					...
					item.damageItem(
							(item.getMaxDamage() - item.getItemDamage()) + 1, player);
					// delete the item
				...
				AxisAlignedBB pool = AxisAlignedBB.getAABBPool().getAABB(
						player.posX - 4.0F, player.posY - 4.0F, player.posZ - 4.0F,
						player.posX + 4.0F, player.posY + 4.0F, player.posZ + 4.0F);
					...
					for (EntityLivingBase el : entl) {
						if (el != null) {
							...
							double potstrength = 1.0D - (Math.sqrt(dist) / (mod_RpgInventory.donators
									.contains(player.getCommandSenderName()) ? 6.0D : 4.0D));
							Potion.heal.affectEntity((EntityLivingBase) player,
									el, (mod_RpgInventory.donators.contains(player
