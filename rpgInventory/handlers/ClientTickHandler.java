...
public class ClientTickHandler implements ITickHandler {
	...
	public void tickEnd(EnumSet<TickType> type, Object... tickData) {
		...
		/**This checks wether the player wears class armor, and a shield, or just a shield (like vanilla shields)*/
		if(Minecraft.getMinecraft().thePlayer != null){
			...
			for (ItemStack is : player.inventory.armorInventory) {
				if (is == null ){
					...
					//if there is one item that is no AbstractArmor, skip the setting of the playerclass
			...
			if(!skip){
				if(player.inventory.getStackInSlot(HELM) != null &&
					...
					String classname = ((AbstractArmor)player.inventory.getStackInSlot(HELM).getItem()).armorClassName();

					mod_RpgInventory.playerClass = classname;

					if(PlayerRpgInventory.get(player).getShield() != null){
						if(((ItemRpgInvArmor)PlayerRpgInventory.get(player).getShield().getItem()).boundArmorClass().equals(classname))
							mod_RpgInventory.playerClass = classname + 
							((ItemRpgInvArmor)PlayerRpgInventory.get(player).getShield().getItem()).shieldClass();
					}
				}else{
					mod_RpgInventory.playerClass = "none";
					if(((ItemRpgInvArmor)PlayerRpgInventory.get(player).getShield().getItem()).boundArmorClass().equals("none")){
						if(PlayerRpgInventory.get(player).getShield() != null){
							mod_RpgInventory.playerClass = ((ItemRpgInvArmor)PlayerRpgInventory.get(player).getShield().getItem()).shieldClass();
						}
					}
				}
			}
