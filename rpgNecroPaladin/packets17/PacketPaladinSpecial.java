	...
	public void handleServerSide(EntityPlayer player) {
		...
		if (CommonTickHandler.globalCooldownMap.get(player.getDisplayName()) <= 0) {
			CommonTickHandler.globalCooldownMap.put(player.getDisplayName(),
					(mod_RpgInventory.donators.contains(player.getDisplayName()) ? 5
							: 7) * 20);
			...
			if ((weapon.getItemDamage() + 3) >= weapon.getMaxDamage()) {
				...
				weapon.damageItem(
						(weapon.getMaxDamage() - weapon.getItemDamage()) + 1, player);
				// delete the item
			...
			AxisAlignedBB pool = AxisAlignedBB.getAABBPool().getAABB(
					player.posX - rad, player.posY - rad, player.posZ - rad, player.posX + rad,
					player.posY + rad, player.posZ + rad);
			...
			player.addChatMessage(new ChatComponentText("You must wait for energy to replenish, left: "
					+ Math.floor(1 + (CommonTickHandler.globalCooldownMap.get(player
